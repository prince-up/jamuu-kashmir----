...
const Mentors = () => {
  ...
  const [chatMessages, setChatMessages] = useState<{from: 'user'|'mentor', text: string}[]>([]);
  const [selectedMentor, setSelectedMentor] = useState(null as null | typeof mentors[0]);
  const [showDialog, setShowDialog] = useState(false);
  // Booking form state
  const [form, setForm] = useState({ date: '', time: '', slot: '', venue: '', mode: 'Online', message: '' });
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setChatMessages(msgs => [
      ...
      { from: 'mentor', text: `For further discussion, please contact me at: \nPhone: 9876543210 \nEmail: mentor@email.com` }
    ]);
    setChatInput('');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-pink-50 to-yellow-50 py-10">
      <h1 className="text-4xl font-extrabold mb-4 text-center bg-gradient-to-r from-blue-600 via-pink-500 to-yellow-500 bg-clip-text text-transparent drop-shadow-lg">Meet Our Mentors</h1>
      <p className="text-center text-lg text-muted-foreground mb-10 max-w-2xl mx-auto">Our handpicked mentors are experts in their fields, passionate about guiding students to success. Connect and learn from the best!</p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-2 md:px-0">
        {mentors.map((mentor, idx) => (
          ...
          >
            <div className="absolute right-0 top-0 opacity-10 text-9xl select-none pointer-events-none">🎓</div>
            <CardHeader className="flex flex-row items-center gap-4">
              <Avatar className="h-14 w-14 border-2 border-white shadow-lg">
                {mentor.avatar ? (
                  <img src={mentor.avatar} alt={mentor.name} className="h-14 w-14 rounded-full object-cover" />
                ) : (
                  <AvatarFallback>{mentor.name.split(' ').map(n => n[0]).join('').slice(0,2)}</AvatarFallback>
                )}
              </Avatar>
              <div>
                <CardTitle className="text-2xl font-bold drop-shadow-sm flex items-center gap-2">
                  {mentor.name}
                  <span className="ml-2 px-2 py-1 rounded-full bg-white/30 text-xs font-semibold uppercase tracking-wide shadow">{mentor.subject}</span>
                </CardTitle>
                <div className="text-sm font-medium opacity-90">{mentor.qualification}</div>
              </div>
            </CardHeader>
            <CardContent>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <Button variant="default" className="bg-white text-blue-600 hover:bg-blue-100 border border-blue-400 shadow font-semibold" onClick={() => handleChat(mentor)}>
                  💬 Chat
                </Button>
      {/* Chat Dialog */}
          ...
          <div className="flex flex-col gap-2 max-h-60 overflow-y-auto mb-2">
            {chatMessages.map((msg, i) => (
              <div key={i} className={msg.from === 'user' ? 'text-right' : 'text-left'}>
                <span className={msg.from === 'user' ? 'inline-block bg-blue-100 text-blue-800 px-3 py-1 rounded-lg mb-1' : 'inline-block bg-gray-100 text-gray-800 px-3 py-1 rounded-lg mb-1'}>
                  {msg.text}
                </span>
              </div>
            ))}
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent>
          ...
          <form className="space-y-3">
            ...
            <Input placeholder="Slot (e.g. 10:00-11:00 AM)" value={form.slot} onChange={e => setForm(f => ({ ...f, slot: e.target.value }))} required />
            <Input placeholder="Venue (e.g. Zoom, Google Meet, or address)" value={form.venue} onChange={e => setForm(f => ({ ...f, venue: e.target.value }))} required />
